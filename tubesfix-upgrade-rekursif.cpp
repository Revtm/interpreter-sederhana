...
float hasiloperasi_rek(stack<float>ar, stack<char>op){
  float E,E2;
  if(op.empty()){
    stack_aritmatika.pop();
    return ar.top();
    ...
    if(op.top() == '+'){
      ...
      op.pop();
      stack_op.pop();
      return (hasiloperasi_rek(ar,op)+E);
    }else if(op.top() == '-'){
      E= ar.top();
      ...
      stack_aritmatika.pop();
      stack_aritmatika.pop();
      stack_aritmatika.push(E);
      ar.push(E);

      op.pop();
      stack_op.pop();

      return (hasiloperasi_rek(ar,op));
    }else{
      E= ar.top();
      ar.pop();
      E2 = ar.top();
  ...
  if(isStuck(state) == true || isFinal(state) == false ){ // jika terjadii stuck atau tidak final
    ...
    if(isEnd(state) == true) *selesai = true;  //jika user menginputkan "# selesai;" merubah variabel
      ...
      if(state == 9){
       ...
       float hasil = hasiloperasi_rek(stack_aritmatika, stack_op);
       cout << "> " << hasil << endl;
